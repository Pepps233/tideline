...
import { buildContextBlocksInTransaction } from "./context-blocks.js";
import { listThreadContextBlocks as listThreadContextBlocksInDb } from "./context-blocks.js";
import { mapTranscriptTurnRow } from "../sqlite/rows.js";
import type { TranscriptTurnRow } from "../sqlite/rows.js";
import type {
...
export interface NormalizedCaptureTurnEvent {
  eventId: string;
...
export interface AppendTurnInTransactionInput {
  createdAt: string;
  mediaType: string;
  raw: Buffer;
  threadId: string;
...
export function normalizeCaptureTurnEventInput(
...
export function captureTurnEventInTransaction(input: {
  ...
  const existingReceipt = getProcessedHookReceipt(
    input.db,
    input.event.eventId,
  );
  ...
  switch (input.event.kind) {
    ...
    case "tool_result":
      insertPendingToolEvent(input.db, input.event);
      break;

    case "model_response_complete": {
      const messageText = extractModelResponseText(input.event.payload);
      const flushInput: {
      ...
      const flush = flushPendingToolEvents({
        appendTurnInTransaction: input.appendTurnInTransaction,
        db: input.db,
        ...flushInput,
      });
    ...
    case "prompt_submit": {
      ...
      const userTurn = input.appendTurnInTransaction({
        createdAt: input.event.createdAt,
        mediaType: "text/plain; charset=utf-8",
        raw: Buffer.from(extractPromptText(input.event.payload), "utf8"),
        threadId: input.event.threadId,
        turnRole: "user",
      });
    ...
    case "session_stop": {
      ...
      const checkpointText = extractCheckpointText(input.event.payload);
      const checkpointTurn =
        checkpointText !== undefined
          ? input.appendTurnInTransaction({
              createdAt: input.event.createdAt,
              mediaType: "text/plain; charset=utf-8",
      ...
      if (checkpointTurn) {
        ...
        compactionActiveTurn =
          latestTurnIndex(input.db, input.event.threadId) + 1;
      }
    ...
    default:
      assertNever(input.event.kind);
  }

  if (compactionActiveTurn !== undefined) {
    contextBlockIds = compactEligibleSourceItems({
  ...
  insertProcessedHookReceipt(input.db, receipt);
  return receipt;
}

function getProcessedHookReceipt(
  db: BetterSqlite3.Database,
  eventId: string,
...
function insertProcessedHookReceipt(
  db: BetterSqlite3.Database,
  receipt: CaptureTurnEventReceipt,
): void {
  db.prepare(
...
function insertPendingToolEvent(
  db: BetterSqlite3.Database,
  event: NormalizedCaptureTurnEvent,
): void {
  db.prepare(
...
function flushPendingToolEvents(input: {
  appendTurnInTransaction: (
    input: AppendTurnInTransactionInput,
  ) => StoredTranscriptTurn;
  assistantText?: string;
  ...
  const pendingToolEvents = listPendingToolEvents(input.db, input.threadId);
  ...
  const turn = input.appendTurnInTransaction({
  ...
  if (flushedToolEventIds.length > 0) {
    deletePendingToolEvents(input.db, flushedToolEventIds);
  }
...
function listPendingToolEvents(
  db: BetterSqlite3.Database,
  threadId: string,
): PendingToolEvent[] {
  const rows = db
...
function deletePendingToolEvents(
  db: BetterSqlite3.Database,
  eventIds: string[],
): void {
...
function compactEligibleSourceItems(input: {
  activeTurn: number;
  clock: () => Date | string;
  createdAt: string;
  db: BetterSqlite3.Database;
  ...
  const groups = listEligibleCompactGroups(
    input.db,
    input.threadId,
    input.activeTurn,
  );
...
function listEligibleCompactGroups(
  db: BetterSqlite3.Database,
  threadId: string,
  activeTurn: number,
): string[][] {
...
function createCaptureReceipt(input: {
  appendedTurnIds: string[];
  ...
  return {
    ...
    timeline: {
      turns: listThreadTurnsInTransaction(input.db, input.event.threadId),
...
function listThreadTurnsInTransaction(
  db: BetterSqlite3.Database,
  threadId: string,
): StoredTranscriptTurn[] {
  const rows = db
...
function latestTurnIndex(db: BetterSqlite3.Database, threadId: string): number {
  const row = db
